...
toSudoku ::[[Val]] ->Sudoku
toSudoku xss = filter ((/=emptyVal).snd) (zip allpos (concat xss)) -- (filter alles wat zinnig is) (concateneer input, en zip deze met alle posities om [(pos, val)] lijst te krijgen)
