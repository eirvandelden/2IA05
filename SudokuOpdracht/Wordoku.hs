...
-- sudoku-spelregel
-- Check voor deze twee posities of ze in hetzelfde rij, kolom of blok zitten.
incident :: Pos -> Pos -> Bool
incident p q = row p q || column p q || block p q

-- Geef die waardes die niet toegestaan zijn op die positie
forbid :: Sudoku -> Pos -> [Val]
forbid s p =  map snd (filter (incident p . fst) s)  

-- Gegeven een Sudoku en een positie, geef de toegestane waardes voor die positie
