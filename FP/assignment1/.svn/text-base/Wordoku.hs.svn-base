...
toSudoku ::[[Char]] ->Sudoku
toSudoku xss = filter ((/=emptyVal).snd) (zip allpos (map (charToInt) (concat xss)))

-- naar rijsgewijze opsomming van val-waarden, uitsluitend complete sudokus
fromSudoku :: Sudoku -> [[Char]]
fromSudoku =  chop 9 . map (intToChar.snd) . sort lexord
